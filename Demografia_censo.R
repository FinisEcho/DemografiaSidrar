...
acre_idade2010_simples <- acre_idade2010_filtrado |> 
  filter(grepl("^[0-9]+ anos?$", Idade)) |>   # pega "1 ano", "2 anos", ..., "99 anos"
  mutate(Idade = as.numeric(sub(" anos?$", "", Idade))) |> 
...
digito_final_2010 <- acre_idade2010_simples$Idade %% 10
...
print("Idade Mediana por Sexo (Acre - Censo 2010):")
print(idade_mediana_acre_2010)

# O dataframe `acre_idade2010_simples` já está pronto com as colunas:
# Ano, Sexo, Idade, Populacao
...
print("Razão de Dependência por Sexo (Acre - Censo 2010):")
print(razao_dependencia_acre_2010)
...
print("Índice de Envelhecimento por Sexo (Acre - Censo 2010):")
print(indice_envelhecimento_acre_2010)

# Vamos criar faixas etárias de 5 em 5 anos
piramide_data <- acre_idade2010_simples |> 
  mutate(
    faixa_etaria = cut(
      x = Idade,
      breaks = c(seq(0, 100, by = 5), Inf),
...
ggplot(piramide_data, aes(x = faixa_etaria, y = Populacao, fill = Sexo)) +
  ...
  coord_flip() + # Inverte os eixos para que a pirâmide seja horizontal
  labs(
    title = "Pirâmide Etária do Acre (Censo 2010)",
    subtitle = "População por sexo e faixas etárias de 5 anos",
    x = "Faixa Etária",
    fill = "Sexo"
  ) +
